#' 2022-02-10
#' Script of the analysis employed to produce the results shown in figure 2 of 
...
# Function call for different data sets generated with an SIRD model 
# and varying levels of relative noise

for (i in c("01","05","1","2","3")){
  
  # Loading of data generated using an SIRD Model with increasing levels of 
  # gaussian noise in the incidence data
  load(paste0("SIR_flux_sim_data_Jacques_all_relE=",i,".RData"))
  dummy <- SIRdatasim_para_R("sim6")
  result[[c]] <- dummy[[1]]
  error[[c]] <- dummy[[2]]
  meas[[c]] <- dummy[[3]]
  c <- c+1
...
save(result, file = "Result_simulations_SIRD_noise.RData")
save(error, file = "Error_simulations_SIRD_noise.RData")
save(meas, file = "Meas_simulations_SIRD_noise.RData")
