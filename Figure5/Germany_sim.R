#' 2022-02-10
#' Script of the analysis employed to produce the results shown in figure 5 of 
...
#'
#'
#' @param blob data.frame, data set supposed to be analyzed by the AKS method
#' @return list of "smooth" (data.frame, output of the smoothed estimates and 
#' the roots of the traces of the smoothed covariance matrices,
#' e.g. the standard deviations of the state-space vector components,as estimated
#' using the AKS method).
#' 

SIRdatasim_Ger_param <- function(blob){
  
  # SIRD model mit fluxes
  ...
  # Data processing
  data <- data.frame( dI=rep(NA,length(blob$time)), dR=rep(NA,length(blob$time)),dD=rep(NA,length(blob$time)), time=rep(NA,length(blob$time)))
  ...
  # After day 551 of the pandemic, the JHU stopped publishing Recovered data, 
  # which is why we only use the data up to that point.
  data <- data[1:551,]
  
  # Number of observations
  NumObs <- length(data$dI)
  ...
  x0 <- c(1000,0,0,0.1,0.1,0.1,log(0.01),log(0.01),log(6))
  
  # Original guess for state-space covariance matrix
  u0 <- c( mean(data$dI),mean(data$dR),mean(data$dD), mean(data$dI),mean(data$dR),mean(data$dD),0.01+exp(x0[7]),0.01+exp(x0[8]),1)
  P0 <- 1*diag(u0)
  ...
  R <-diag(c(mean(data$dI),mean(data$dR),mean(data$dD)))
  ...
  system.time(
    while (epsilon>1e-3) {
      ...
      for(i in 1:NumObs){
        ...
        # Observation
        y <- c(data$dI[i],data$dR[i],data$dD[i])
      ...
      # Calculations for M-step start here 
      
      summ <- 0
      for (i in time[-1]){ summ<- summ+(c(data$dI[i],data$dR[i],data$dD[i]) - G%*%smooth_list[[i]])%*%
        t(c(data$dI[i],data$dR[i],data$dD[i]) - G%*%smooth_list[[i]]) + G%*%P_smooth[[i]]%*%t(G)}
      ...
      smooth$time <- data$time
      smooth <- cbind(smooth,std)
...
dummy <- SIRdatasim_Ger_param(blob = data)

results <- dummy[[1]]

save(results, file = "Result_Germany.RData")