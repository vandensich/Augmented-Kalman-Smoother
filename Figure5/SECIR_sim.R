#' 2022-02-10
#' Script of the analysis employed to produce the results shown in figure 4 of 
...
#' "Meas" (data.frame, analyzed noisy simulated
#' data upon which the estimations of smooth are based), "data" (
#' data.frame, simulated parameter and state time courses).

SIRdatasim_para_R_SECIR <- function(sim="sim1"){
  ...
  if (sim == "sim1"){
    data <- mydata$sim1
    pred <- prediction$sim1
  }
  
  if (sim == "sim2"){
    data <- mydata$sim2
    pred <- prediction$sim2
  }
  ...
  u0 <- c(mean(Meas$dI),mean(Meas$dR),mean(Meas$dD), mean(Meas$dI),mean(Meas$dR),mean(Meas$dD),0.01+exp(x0[7]),0.01+exp(x0[8]),1)
  P0 <- 1*diag(u0)
  ...
  return(list(smooth,data,Meas))
  
}

results <- list()

# Loading of data generated using an SECIR Model with different parameter time
# courses
load("SECIR_sim_data.RData")
load("SECIR_sim_pred.RData")

# Function call for the analysis of the loaded with the AKS method
dummy <-SIRdatasim_para_R_SECIR()

results <- dummy[[1]]
data <- dummy[[2]]
Meas<- dummy[[3]]

save(results, file = "Result_SECIR.RData")
save(Meas, file = "Meas_SECIR.RData")
save(data, file = "Data_SECIR.RData")